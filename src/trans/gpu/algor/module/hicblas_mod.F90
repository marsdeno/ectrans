...
USE PARKIND1, ONLY : JPIM, JPRM, JPRD, JPIB
...
INTERFACE
    ...
    &) BIND(C, NAME='hipblas_dgemm_wrapper')
        ...
        REAL(C_DOUBLE),      VALUE            :: ALPHA,BETA
        REAL(C_DOUBLE),      DIMENSION(LDA,*), DEVICE :: A
        REAL(C_DOUBLE),      DIMENSION(LDB,*), DEVICE :: B
        REAL(C_DOUBLE),      DIMENSION(LDC,*), DEVICE :: C
...
INTERFACE
    ...
    &) BIND(C, NAME='hipblas_sgemm_wrapper')
        ...
        INTEGER(C_INT),      VALUE            :: M, N, K, LDA, LDB, LDC, TDA, TDB, TDC, BATCHCOUNT
        REAL(C_FLOAT),       VALUE            :: ALPHA, BETA
        REAL(C_FLOAT),       DIMENSION(LDA,*), DEVICE :: A
        REAL(C_FLOAT),       DIMENSION(LDB,*), DEVICE :: B
        REAL(C_FLOAT),       DIMENSION(LDC,*), DEVICE :: C
        INTEGER(KIND=C_SIZE_T) :: STREAM
        TYPE(C_PTR), INTENT(IN), VALUE :: ALLOC
...
SUBROUTINE HIP_DGEMM_BATCHED_OVERLOAD( &
    ...
    REAL(KIND=JPRD), DIMENSION(:), DEVICE :: AARRAY
    INTEGER(KIND=JPIM) :: LDA
    INTEGER(KIND=JPIM) :: STRIDEA
    REAL(KIND=JPRD), DIMENSION(:,:), DEVICE :: BARRAY
    INTEGER(KIND=JPIM) :: LDB
    INTEGER(KIND=JPIM) :: STRIDEB
    REAL(KIND=JPRD) :: BETA
    REAL(KIND=JPRD), DIMENSION(:), DEVICE :: CARRAY
  ...
  SUBROUTINE HIP_SGEMM_BATCHED_OVERLOAD( &
    ...
    REAL(KIND=JPRM), DIMENSION(:), DEVICE :: AARRAY
    INTEGER(KIND=JPIM) :: LDA
    INTEGER(KIND=JPIM) :: STRIDEA
    REAL(KIND=JPRM), DIMENSION(*), DEVICE :: BARRAY
    INTEGER(KIND=JPIM) :: LDB
    INTEGER(KIND=JPIM) :: STRIDEB
    REAL(KIND=JPRM) :: BETA
    REAL(KIND=JPRM), DIMENSION(:), DEVICE :: CARRAY
    INTEGER(KIND=JPIM) :: LDC
    INTEGER(KIND=JPIM) :: STRIDEC
    INTEGER(KIND=JPIM) :: BATCHCOUNT
    INTEGER(KIND=C_INT) :: STREAM
