...
MODULE TPM_HICFFT
  ...
  SUBROUTINE CREATE_PLAN_FFT(KPLAN,KTYPE,KN,KLOT,KSTRIDE)
    ...
    DO JL=1,HICT%N_PLANS(KN)
      IF( KLOT == CURR_HICFFT_PLAN%NLOT .AND. KTYPE == CURR_HICFFT_PLAN%NTYPE .AND. KSTRIDE == CURR_HICFFT_PLAN%NSTRIDE)THEN
        LLFOUND=.TRUE.
    ...
    IF( .NOT.LLFOUND )THEN
      IF( LLRESTRICT_PLANS )THEN
        IF( HICT%N_PLANS(KN) == HICT%N_MAX_PLANS )THEN
          ! destroy the plan at the start of the list
          CALL DESTROY_PLAN_FFT(HICT%HICFFT_PLANS(KN)%NPLAN)
          HICT%HICFFT_PLANS(KN)%NPLAN_ID=999999
          START_HICFFT_PLAN=>HICT%HICFFT_PLANS(KN)
          HICT%HICFFT_PLANS(KN)=HICT%HICFFT_PLANS(KN)%NEXT_PLAN
          ! DEALLOCATE(START_HICFFT_PLAN)
