...
CONTAINS
  FUNCTION PREPARE_TRLTOM(ALLOCATOR, KF_FS) RESULT(HTRLTOM)
    ...
    HTRLTOM%HPFBUF = RESERVE(ALLOCATOR, 2_JPIB*D%NLENGT1B*KF_FS*SIZEOF(DUMMY), "HTRLTOM%HPFBUF")
  END FUNCTION
...
#define TRLTOM_DTYPE MPI_DOUBLE
    ...
    IF(NPROC > 1) THEN
      ...
      IF (ILENS(IRANK) > 0) THEN
          ...
          DO J = 1, TO_SEND - FROM_SEND + 1
            PFBUF(FROM_RECV+J-1) = PFBUF_IN(FROM_SEND+J-1)
          ENDDO
#endif
          ILENS(IRANK) = 0
          ILENR(IRANK) = 0
