...
CONTAINS
  ...
  SUBROUTINE TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    LOGICAL, OPTIONAL, INTENT(IN) :: LD_ON_DEVICE_ARRAY(11)
...
#define TRGTOL_DTYPE MPI_DOUBLE
    ...
    !     ------------------------------------------------------------------
    IF (PRESENT(LD_ON_DEVICE_ARRAY)) THEN
      LL_ON_DEVICE_ARRAY(:) = LD_ON_DEVICE_ARRAY(:)
    ELSE
      LL_ON_DEVICE_ARRAY(:) = .FALSE.
    ENDIF
    ...
    CALL GSTATS(412,0)
    ACC_POINTERS_CNT = 0
    IF (PRESENT(PGP) .AND. .NOT. ACC_IS_PRESENT(PGP)) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGP)
      LL_ON_DEVICE_ARRAY(7) = .FALSE.
      WRITE(NOUT,*) 'dir_trans, PGP not on device'
    ENDIF
    IF (PRESENT(PGPUV) .AND. .NOT. ACC_IS_PRESENT(PGPUV)) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGPUV)
      LL_ON_DEVICE_ARRAY(8) = .FALSE.
      WRITE(NOUT,*) 'dir_trans, PGPUV not on device'
    ENDIF
    IF (PRESENT(PGP2) .AND. .NOT. ACC_IS_PRESENT(PGP2)) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGP2)
      LL_ON_DEVICE_ARRAY(11) = .FALSE.
      WRITE(NOUT,*) 'dir_trans, PGP2 not on device'
    ENDIF
    IF (PRESENT(PGP3A) .AND. .NOT. ACC_IS_PRESENT(PGP3A)) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGP3A)
      LL_ON_DEVICE_ARRAY(9) = .FALSE.
      WRITE(NOUT,*) 'dir_trans, PGP3A not on device'
    ENDIF
    IF (PRESENT(PGP3B) .AND. .NOT. ACC_IS_PRESENT(PGP3B)) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGP3B)
      LL_ON_DEVICE_ARRAY(10) = .FALSE.
      WRITE(NOUT,*) 'dir_trans, PGP3B not on device'
    ENDIF
    IF (ACC_POINTERS_CNT > 0) CALL EXT_ACC_CREATE(ACC_POINTERS(1:ACC_POINTERS_CNT),STREAM=1_ACC_HANDLE_KIND)
    !$ACC WAIT(1)
    IF (PRESENT(PGP) .AND. .NOT. LL_ON_DEVICE_ARRAY(7)) THEN
#ifdef OMPGPU
#endif
#ifdef ACCGPU
        WRITE(NOUT,*) 'dir_trans, updating PGP on device'
        !$ACC UPDATE DEVICE(PGP)
#endif
    ENDIF
    IF (PRESENT(PGPUV) .AND. .NOT. LL_ON_DEVICE_ARRAY(8)) THEN
#ifdef OMPGPU
#endif
#ifdef ACCGPU
        WRITE(NOUT,*) 'dir_trans, updating PGPUV on device'
        !$ACC UPDATE DEVICE(PGPUV)
#endif
    ENDIF
    IF (PRESENT(PGP2) .AND. .NOT. LL_ON_DEVICE_ARRAY(11)) THEN
#ifdef OMPGPU
#endif
#ifdef ACCGPU
        WRITE(NOUT,*) 'dir_trans, updating PGP2 on device'
        !$ACC UPDATE DEVICE(PGP2)
#endif
    ENDIF
    IF (PRESENT(PGP3A) .AND. .NOT. LL_ON_DEVICE_ARRAY(9)) THEN
#ifdef OMPGPU
#endif
#ifdef ACCGPU
        WRITE(NOUT,*) 'dir_trans, updating PGP3A on device'
        !$ACC UPDATE DEVICE(PGP3A)
#endif
    ENDIF
    IF (PRESENT(PGP3B) .AND. .NOT. LL_ON_DEVICE_ARRAY(10)) THEN
#ifdef OMPGPU
#endif
#ifdef ACCGPU
        WRITE(NOUT,*) 'dir_trans, updating PGP3B on device'
