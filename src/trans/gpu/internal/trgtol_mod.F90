...
#define TRGTOL_DTYPE MPI_DOUBLE_PRECISION
  ...
  !$ACC KERNELS DEFAULT(NONE) PRESENT(ZCOMBUFS,ZCOMBUFR) ASYNC(1)
#endif
#ifdef OMPGPU
...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(LLPGPONLY) THEN
    ...
    !$ACC& PRESENT(NGPBLKS,IFLDS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF,KINDEX,PGLAT,PGP) ASYNC(1)
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
        DO JKL=1, JK_MAX
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) COPYIN(IRECV,ILEN) &
