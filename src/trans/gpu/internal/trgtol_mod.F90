...
#define TRGTOL_DTYPE MPI_DOUBLE_PRECISION
  ...
  ELSE
    ALLOCATE(ZCOMBUFS(1,1))
  ENDIF
  IF (IBUFLENR > 0) THEN
    ALLOCATE(ZCOMBUFR(IRECVCOUNT,INRECV))
  ELSE
    ALLOCATE(ZCOMBUFR(1,1))
  ENDIF

#ifdef ACCGPU
  !$ACC DATA CREATE(ZCOMBUFS,ZCOMBUFR) ASYNC(1)
...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
...
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) FIRSTPRIVATE(NGPBLKS,KF_FS,JK_MAX,MYSETW,MYPROC) ASYNC(1)
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
        DO JKL=1, JK_MAX
  ...
  ELSE
    ...
    !$ACC&        IGP2PARS,IGP3ALEVS,IGP3APARS) ASYNC(1)
#elif defined(_CRAYFTN)
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
    !$ACC  COPYIN(NGPBLKS,IFLDS,JK_MAX,MYSETW,MYPROC,KF_FS) &
    !$ACC& PRESENT(IGPTRSEND,INDOFF,IGPTROFF,IFLDOFF, &
    !$ACC&        LLUV,LLGP2,LLGP3A,LLGP3B,PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B, &
    !$ACC&        IGP2PARS,IGP3ALEVS,IGP3APARS) ASYNC(1) !!,IUVLEVS,IUVPARS,IGP3BLEVS,IGP3BPARS)
#endif
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
      ...
      !$ACC  COPYIN(NGPBLKS,ISEND_FLD_CNT,INS,JK_MAX,IJPOS,IFLDA,IPOS,ISETW,LLINDER,LLPGPONLY) &
      !$ACC& PRESENT(IGPTRSEND,LLUV,LLGP2,LLGP3A,LLGP3B,ZCOMBUFS,KPTRGP,PGP,PGPUV,PGP2,PGP3A,PGP3B, &
      !$ACC&        IGP2PARS,IGP3ALEVS,IGP3APARS) ASYNC(1) !!,IUVLEVS,IUVPARS,IGP3BLEVS,IGP3BPARS)
#endif
#endif
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) FIRSTPRIVATE(KF_FS,ILEN,INDOFFL,INR) &
      !$ACC& PRESENT(KINDEX,INDOFF,ZCOMBUFR,PGLAT) ASYNC(1)
#endif
      DO JFLD=1,KF_FS
        DO JL=1,ILEN
...
#ifdef ACCGPU
   !$ACC END DATA  !! ZCOMBUFS ZCOMBUFR
#endif
