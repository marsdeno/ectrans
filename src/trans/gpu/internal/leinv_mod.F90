...
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,ISL,JGL,OFFSET1,OFFSET2,ZAOA) DEFAULT(NONE)
#endif
DO KMLOC=1,D_NUMP
  DO JK=1,2*KF_LT
    KM = D_MYMS(KMLOC)
    ...
    DO JGL=ISL,R_NDGNH
      ...
      IF(KM /= 0) THEN
        ZAOA = ZOUT(JK+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_LT)
      ELSEIF (MOD((JK-1),2) .EQ. 0) THEN
        ZAOA = ZOUT((JK-1)/2+1+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_LT)
      ENDIF
      FOUBUF_IN(OFFSET1+JK) = ZAOA
    ENDDO
  ENDDO
ENDDO
...
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,ISL,IGLS,ISTAS,ZAOA,ZSOA) DEFAULT(NONE)
#endif
DO KMLOC=1,D_NUMP
  DO JK=1,2*KF_LT
    KM = D_MYMS(KMLOC)
    ...
    DO JGL=ISL,R_NDGNH
      ...
      IF(KM /= 0) THEN
        ZSOA = ZOUT(JK+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_LT)
      ELSEIF (MOD((JK-1),2) .EQ. 0) THEN
        ZSOA = ZOUT((JK-1)/2+1+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_LT)
      ENDIF

      ZAOA = FOUBUF_IN(OFFSET1+JK)
