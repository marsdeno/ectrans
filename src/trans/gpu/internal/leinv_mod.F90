  ...
  DO JK=1,KFC
    ...
    IF(KM /= 0)THEN
      ...
      DO J=1,(R_NSMAX-KM+2)/2
        ...
        ZZBA((JK-1)+1,J,KMLOC)=PIA(JK,IA+1+(J-1)*2,KMLOC)
      ENDDO
...
#endif
#ifdef OMPGPU
!$OMP TARGET DATA USE_DEVICE_PTR(ZAA,ZZBA,ZZCSTA)
#endif
...
#endif
#ifdef ACCGPU
!$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(KM,ISL,KDGLU) DEFAULT(NONE)
#endif
DO KMLOC=1,D_NUMP
  DO JK=1,KFC
    KM = D_MYMS(KMLOC)
  ...
  DO JK=1,KFC
    ...
    IF(KM /= 0)THEN
      ...
      DO J=1,(R_NSMAX-KM+3)/2
        ...
        ZZBS((JK-1)+1,J,KMLOC)=PIA(JK,IS+1+(J-1)*2,KMLOC)
      ENDDO
    ELSEIF (MOD((JK-1),2) .EQ. 0) THEN
      !$ACC LOOP SEQ
      DO J=1,(R_NSMAX+3)/2
