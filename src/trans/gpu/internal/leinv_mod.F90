...
#ifdef ACCGPU
!$ACC DATA PRESENT(PAOA1,PSOA1,D_MYMS,D_NPROCL,D_NSTAGT1B,D_NPNTGTB1,G_NDGLU) PRESENT(FOUBUF_IN)
#endif
#ifdef ACCGPU
...
DO KMLOC=1,D_NUMP
  DO JF=1,2*KF_OUT_LT
    ...
    DO JGL=ISL, R_NDGNH
      IGLS = R_NDGL+1-JGL
      OFFSET1 = (D_NSTAGT1B(D_NPROCL(JGL)) + D_NPNTGTB1(KMLOC,JGL))*2*KF_OUT_LT
      OFFSET2 = (D_NSTAGT1B(D_NPROCL(IGLS)) + D_NPNTGTB1(KMLOC,IGLS))*2*KF_OUT_LT

      FOUBUF_IN(OFFSET1+JF) = PAOA1(JF,JGL,KMLOC)+PSOA1(JF,JGL,KMLOC)
      FOUBUF_IN(OFFSET2+JF) = PSOA1(JF,JGL,KMLOC)-PAOA1(JF,JGL,KMLOC)
    ENDDO
  ENDDO
ENDDO
!$ACC END DATA
