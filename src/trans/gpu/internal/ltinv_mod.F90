...
CONTAINS
  ...
  SUBROUTINE LTINV(ALLOCATOR,HLTINV,KF_UV,KF_SCALARS,&
     ...
     & ZOUTS,ZOUTA,ZOUTS0,ZOUTA0, LD_ON_DEVICE_ARRAY)
...
#endif
#ifdef ACCGPU
    !$ACC DATA COPYIN(PSPVOR,PSPDIV) IF(KF_UV > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(1))
    !$ACC DATA COPYIN(PSPSCALAR) IF(PRESENT(PSPSCALAR) .AND. KF_SCALARS > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(3))
    !$ACC DATA COPYIN(PSPSC2) IF(NF_SC2 > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(6))
    !$ACC DATA COPYIN(PSPSC3A) IF(NF_SC3A > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(4))
    !$ACC DATA COPYIN(PSPSC3B) IF(NF_SC3B > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(5))
#endif
    IF (LSYNC_TRANS) THEN
      CALL GSTATS(442,0)
      CALL MPL_BARRIER(CDSTRING='')
