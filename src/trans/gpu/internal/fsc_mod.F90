...
REAL(KIND=JPRBT) :: ZMUL, ZACHTE2, ZSHIFT, ZPI
REAL(KIND=JPRBT) :: ZAMP, ZPHASE
INTEGER(KIND=JPIM) :: IOFF, OFFSET_VAR
INTEGER(KIND=JPIM) :: JF,IGLG,II,JM,IST
...
IF (ZGTF_START(ZGTF_START_INDEX_UV) /= ZGTF_START(ZGTF_START_INDEX_UV+1)) THEN
  ...
  DO KGL=IBEG,IEND,IINC
    DO JF=ZGTF_START(ZGTF_START_INDEX_UV),ZGTF_START(ZGTF_START_INDEX_UV+1)-1
      ...
      !$ACC LOOP SEQ
      DO JM=0,2*G_NMEN(IGLG)
        ZGTF(2*JF-1,JM+IOFF) = ZGTF(2*JF-1,JM+IOFF)*ZACHTE2
        ZGTF(2*JF,  JM+IOFF) = ZGTF(2*JF  ,JM+IOFF)*ZACHTE2
      ENDDO
    ENDDO
  ENDDO
ENDIF
