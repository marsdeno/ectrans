...
MODULE LTDIR_MOD
  USE PARKIND_ECTRANS,        ONLY: JPIM, JPRBT, JPRB, JPRD
...
CONTAINS
  ...
  SUBROUTINE LTDIR(ALLOCATOR,HLTDIR,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,KF_SCALARS,&
     ...
     & KFLDPTRUV,KFLDPTRSC, LD_ON_DEVICE_ARRAY)
...
#endif
#ifdef ACCGPU
    !$ACC DATA COPYOUT(PSPVOR,PSPDIV) IF(KF_UV > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(1))
    !$ACC DATA COPYOUT(PSPSCALAR) IF(PRESENT(PSPSCALAR) .AND. KF_SCALARS > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(3))
    !$ACC DATA COPYOUT(PSPSC2) IF(NF_SC2 > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(6))
    !$ACC DATA COPYOUT(PSPSC3A) IF(NF_SC3A > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(4))
    !$ACC DATA COPYOUT(PSPSC3B) IF(NF_SC3B > 0 .AND. .NOT. LL_ON_DEVICE_ARRAY(5))
#endif
