...
#ifdef ACCGPU
!$ACC DATA PRESENT(FOUBUF_IN,ZGTF,D_NPTRLS,G_NMEN,D_NPROCM,D_NSTAGT0B,D_NPNTGTB0,D_NSTAGTF)
#endif
#ifdef OMPGPU
!WARNING: following line should be PRESENT,ALLOC but causes issues with AMD compiler!
!$OMP TARGET DATA MAP(ALLOC:FOUBUF_IN,ZGTF, D_NPTRLS,G_NMEN,D_NPROCM,D_NSTAGT0B,D_NPNTGTB0,D_NSTAGTF)

!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) COLLAPSE(2) PRIVATE(IGLG,IPROC,ISTA,IOFF,JM) &
!$OMP&    SHARED(IINC,G_NMEN_MAX,KFIELDS,D_NPTRLS,MYSETW,G_NMEN, &
!$OMP&           D_NPROCM,D_NSTAGT0B,D_NPNTGTB0,D_NSTAGTF,FOUBUF_IN,ZGTF)
#endif
#ifdef ACCGPU
