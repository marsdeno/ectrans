...
ALLOCATE(ZINP(2*KF_FS*R_NDGNH*D_NUMP))
...
#ifdef ACCGPU
!$ACC DATA &
!$ACC& CREATE(ZINP,ZOUT,ZINP0,ZOUT0) &
...
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP DEFAULT(NONE) COLLAPSE(2) PRIVATE(KM,ISL,IGLS,OFFSET1,OFFSET2,JGL,PAIA)
#endif
  DO KMLOC=1,D_NUMP
    DO JF=1,KF_FS*2
      KM = D_MYMS(KMLOC)
      ...
      DO JGL=ISL,R_NDGNH
        ...
        ENDIF
        ZINP(JF+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_FS)=PAIA*F_RW(JGL)
      ENDDO
    ENDDO
  ENDDO
...
! symmetric
    ...
    DO JF=1,KF_FS*2
      ...
      DO JGL=ISL,R_NDGNH
        ...
        IF (JF .LE. 4*KF_UV) THEN
          PAIA = PAIA*F_RACTHE(JGL)
        ENDIF
        ZINP((JF-1)+1+(JGL-ISL+(KMLOC-1)*R_NDGNH)*2*KF_FS)=PAIA*F_RW(JGL)
      ENDDO
    ENDDO
  ENDDO
  ...
  ! C^T=B^T*A^T
#ifdef OMPGPU
  !$OMP TARGET DATA USE_DEVICE_PTR(ZAS,ZINP,ZOUT)
#endif
#ifdef ACCGPU
