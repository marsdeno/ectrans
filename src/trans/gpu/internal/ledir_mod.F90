...
IF ( KMODE == -1 ) THEN
  ...
  !$OMP&     SHARED(D_NUMP,R_NDGNH,D_MYMS,G_NDGLU,DZBST,PAIA,F_RW)
#endif
#ifdef ACCGPU
  ...
  DO KMLOC=1,D_NUMP
    DO JK=1,2*KF_FS
      ...
      IF (KM /= 0) THEN
        ...
        !$ACC LOOP SEQ
        DO J=1,G_NDGLU(KM)
          DZBST((JK-1)+1+(J-1+(KMLOC-1)*R_NDGNH)*2*KF_FS)=PAIA(JK,ISL+J-1,KMLOC)*F_RW(ISL+J-1)
        ENDDO
      ENDIF
    ENDDO
  ENDDO
  ...
  IF(KMLOC0 > 0) THEN
    ...
    DO J=1,G_NDGLU(0)
      DO JK=1,2*KF_FS,2
        ...
        DZBST0((JK-1)/2+1+(J-1)*2*KF_FS)=PAIA(JK,ISL+J-1,KMLOC0)*F_RW(ISL+J-1) 
      ENDDO
    ENDDO
  ...
  DO KMLOC=1,D_NUMP
    DO JK=1,2*KF_FS
      ...
      IF (KM /= 0) THEN
        ...
        !$ACC LOOP SEQ
        DO J=1,G_NDGLU(KM)
          DZBST(JK+(J-1+(KMLOC-1)*R_NDGNH)*2*KF_FS)=PAIA(JK,ISL+J-1,KMLOC)*F_RW(ISL+J-1)
        ENDDO
      ENDIF
    ENDDO
  ENDDO
