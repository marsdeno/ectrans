...
#ifdef ACCGPU
!$ACC DATA                                  &
!$ACC&      PRESENT (F_RN, R_NTMAX, D_MYMS, D_NUMP) &
...
!*       1.1      COMPUTE
  ...
  !!$OMP&   SHARED(KM,ISMAX,F,ZN,ZEPSNM,KMLOC)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP DEFAULT(NONE) COLLAPSE(2) PRIVATE(KM,IR,II,JI) FIRSTPRIVATE(KF_SCALARS)
#endif
DO KMLOC=1,D_NUMP
  DO J=1,KF_SCALARS
    KM = D_MYMS(KMLOC)
