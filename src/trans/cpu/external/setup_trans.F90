...
USE TPM_DISTR       ,ONLY : D, DISTR_RESOL,NPROC,MYSETW
...
! Local variables
INTEGER(KIND=JPIM) :: JGL,JRES,IDEF_RESOL, IGLG, ILATS
...
#endif
#ifdef WITH_FFT992
  ALLOCATE(FFT992_RESOL(NMAX_RESOL))
#endif
...
#ifdef WITH_FFT992
  IF(LDUSEFFTW) THEN
    CALL ABORT_TRANS('SETUP_TRANS: FFTW backend requested but FFTW is not enabled in this build')
  ENDIF
#else
...
#else
  IF(LD_ALL_FFTW) THEN
    CALL ABORT_TRANS('SETUP_TRANS: LD_ALL_FFTW requires FFTW support in this build')
  ENDIF
#endif
ENDIF
...
! Initialize Fast Fourier Transform package
  IF (.NOT. D%LCPNMONLY .AND. .NOT. D%LGRIDONLY) THEN
    ...
    ENDIF
#else
    CALL ABORT_TRANS('SETUP_TRANS: no Fourier backend available for this build configuration')
#endif
  ENDIF
  CALL GSTATS(1802,1)
ELSE
