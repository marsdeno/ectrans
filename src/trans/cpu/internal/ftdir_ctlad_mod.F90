...
! See ftinv_ctl_mod for full rationale.
...
USE TPM_GEN         ,ONLY : NSTACK_MEMORY_TR, LALLOPERM
...
IF (NSTACK_MEMORY_TR == 1) THEN
  ZGTF => ZGTF_STACK(:,:)
ELSE
  ! persistent flat buffer + 2D pointer rank-remap, grows to maximum size needed
...
IF (KF_FS > 0) THEN
  ...
  ! FTDIRAD uses ITYPE=1 (C2R, adjoint of forward R2C).
  ...
  DO JGL = 1, D%NDGL_FS
    ...
    ! Fourier transform
    CALL FTDIRAD(ZGTF, KF_FS, JGL, IPLAN_FFTW(JGL))
  ENDDO
  !$OMP END PARALLEL DO
