...
! Persistent per-thread tile buffer for the local PGP<->PGLAT copy.
...
INTEGER(KIND=JPIM) :: I_ISETW_INS
INTEGER(KIND=JPIM) :: I_JJ_OFF                ! (JJ-ISEND_FLD_START)*IPOS
...
!$OMP&                 ISETV,IFLD,IFLDT,IPOS,JFLD,JK,JJ,JI,I_JJ_OFF,JI_BASE, &
!$OMP&                 I_ISETW_INS,I_KPTRGP_IFLDT,I_LEV_UV,I_PAR_UV,I_PAR_GP2, &
...
!$OMP DO SCHEDULE(STATIC)
  DO JJ=ISEND_FLD_START,ISEND_FLD_END
    IFLDT    = IFLDA(JJ,INS)
    I_JJ_OFF = (JJ-ISEND_FLD_START)*IPOS
    I_ISETW_INS = ISETW(INS)
    ...
    ENDIF
    DO JBLK=1,NGPBLKS
      IFIRST = KGPTRSEND(1,JBLK,I_ISETW_INS)
      IF(IFIRST > 0) THEN
        ILAST   = KGPTRSEND(2,JBLK,I_ISETW_INS)
        JI_BASE = I_JJ_OFF + IJPOS(JBLK,INS) - IFIRST + 1
        IF(LLINDER) THEN
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PCOMBUFS(JI,INS) = PGP(JK,I_KPTRGP_IFLDT,JBLK)
          ENDDO
        ELSE
          IF(LLPGPONLY) THEN
            !DIR$ VECTOR ALWAYS
          ...
          ELSEIF(LLUV(IFLDT)) THEN
            ...
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PCOMBUFS(JI,INS) = PGPUV(JK,I_LEV_UV,I_PAR_UV,JBLK)
            ENDDO
          ELSEIF(LLGP2(IFLDT)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PCOMBUFS(JI,INS) = PGP2(JK,I_PAR_GP2,JBLK)
            ENDDO
          ELSEIF(LLGP3A(IFLDT)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PCOMBUFS(JI,INS) = PGP3A(JK,I_LEV_GP3A,I_PAR_GP3A,JBLK)
            ENDDO
          ELSEIF(LLGP3B(IFLDT)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PCOMBUFS(JI,INS) = PGP3B(JK,I_LEV_GP3B,I_PAR_GP3B,JBLK)
            ENDDO
          ENDIF
        ENDIF
      ENDIF
