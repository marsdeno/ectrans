...
USE TPM_ECTRANS_OPTS,ONLY : LUSE_OPT2, LUSE_OPT3, LUSE_LEVS_TR, LVERIFY_LEVS_NODE, LREPORT_TR_BW, LDUMP_TR_SKEW
...
! once-only per-rank send-payload dump (pack-work proxy) for the G->L
! direction, for cross-checking the TRLTOG skew analysis (guard:
! LDUMP_TR_SKEW).
...
INTEGER(KIND=JPIM)  :: ISENDTOT_UNIT, JRT
...
IF (LREPORT_TR_BW .AND. .NOT. LGPNORM) THEN
  ...
  ! One-shot dump of every rank's send payload (elements) for the G->L
  ! direction, to check whether the grid-side partition is balanced.
  IF (LDUMP_TR_SKEW .AND. .NOT. LSENDTOT_DUMPED) THEN
    ...
    CALL MPL_ALLGATHERV(ZSENDTOT_LOC(1:1), ZSENDTOT_ALL, ISENDTOT_CNTS, &
      &                 CDSTRING='TRGTOL_COMM: SENDTOT GATHER')
    IF (MYPROC == 1) THEN
      OPEN(NEWUNIT=ISENDTOT_UNIT, FILE='tr_sendtot_gtol.csv', STATUS='REPLACE', ACTION='WRITE')
      WRITE(ISENDTOT_UNIT,'(A,I0)') '# per-rank TRGTOL send payload [elements]; nproc=', NPROC
