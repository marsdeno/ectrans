...
USE TPM_ECTRANS_OPTS,ONLY : LUSE_OPT1, LREPORT_STAGE_TIME
!
...
REAL(KIND=JPRD) :: ZT_S107, ZT_S1639, ZT_S157
REAL(KIND=JPRD), EXTERNAL :: TIMEF
REAL(KIND=JPRD), SAVE :: ZT_ACC_107 = 0.0_JPRD, ZT_ACC_1639 = 0.0_JPRD
REAL(KIND=JPRD), SAVE :: ZT_ACC_157 = 0.0_JPRD
...
IF (LREPORT_STAGE_TIME) ZT_S107 = TIMEF()
CALL GSTATS(107,0)
...
IF (LREPORT_STAGE_TIME) ZT_S1639 = TIMEF()
CALL GSTATS(1639,0)
...
!$OMP END PARALLEL DO
CALL GSTATS(1639,1)
IF (LREPORT_STAGE_TIME) ZT_ACC_1639 = ZT_ACC_1639 + (TIMEF() - ZT_S1639)
...
CALL GSTATS(107,1)
IF (LREPORT_STAGE_TIME) ZT_ACC_107 = ZT_ACC_107 + (TIMEF() - ZT_S107)
...
IF (LREPORT_STAGE_TIME) ZT_S157 = TIMEF()
CALL GSTATS(157,0)
CALL TRLTOG(ZGTF,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
CALL GSTATS(157,1)
IF (LREPORT_STAGE_TIME) THEN
  ZT_ACC_157 = ZT_ACC_157 + (TIMEF() - ZT_S157)
  ZT_NCALLS = ZT_NCALLS + 1
  IF (MYPROC == 1 .AND. ZT_NCALLS <= 10) THEN
    WRITE(0,'(A,F12.3,A,F12.3,A,F12.3)') 'FTINV wall: g107=', ZT_ACC_107/ZT_NCALLS, &
     & 'g1639=', ZT_ACC_1639/ZT_NCALLS, 'g157=', ZT_ACC_157/ZT_NCALLS
  ENDIF
ENDIF
