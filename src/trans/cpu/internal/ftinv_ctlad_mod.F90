...
! ZGTF buffer (grow-only flat 1D, pointer-remapped to 2D)
...
IF (NSTACK_MEMORY_TR == 1) THEN
  ZGTF => ZGTF_STACK(:,:)
ELSE
  ! persistent flat buffer + 2D pointer rank-remap, grows to maximum needed size
...
! FTINVAD uses ITYPE=-1 (R2C, adjoint of forward C2R)
IF (KF_FS > 0) THEN
...
DO JGL = 1, D%NDGL_FS
  IGL = JGL
  IF(KF_FS > 0) THEN
    CALL FTINVAD(ZGTF,KF_FS,IGL,IPLAN_FFTW(IGL))
  ENDIF
