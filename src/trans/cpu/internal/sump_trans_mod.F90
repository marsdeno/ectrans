...
IF(.NOT.D%LWEIGHTED_DISTR) THEN
  ...
  CALL SUMPLAT(R%NDGL,NPROC,N_REGIONS_NS,MY_REGION_NS,D%LSPLIT,LEQ_REGIONS,&
             ...
             &LUSE_FOURIER_BOUNDARIES=LLFOURIER_BOUNDS,KPTRLS_FOURIER=D%NPTRLS,KULTPP_FOURIER=D%NULTPP)
ELSE
  LLFOURIER_BOUNDS = LUSE_RECTANGULAR_DECOMP .AND. .NOT. D%LGRIDONLY
  CALL SUMPLAT(R%NDGL,NPROC,N_REGIONS_NS,MY_REGION_NS,D%LSPLIT,LEQ_REGIONS,&
             &D%NFRSTLAT,D%NLSTLAT,D%NFRSTLOFF,D%NPTRLAT,&
...
IF (.NOT. LL_GP_DUMPED .AND. MYPROC == 1) THEN
  ...
  WRITE(IDUMP,'(A,I0,A,I0,A,I0,A,I0)') &
    '# nprgpns=', NPRGPNS, ', nprgpew=', NPRGPEW, ', nreg_ns=', N_REGIONS_NS, ', nreg_ew=', N_REGIONS_EW
  WRITE(IDUMP,'(A)',ADVANCE='NO') '# n_regions='
  DO JA = 1, N_REGIONS_NS
    WRITE(IDUMP,'(I0,A)',ADVANCE='NO') N_REGIONS(JA), ','
  ENDDO
  WRITE(IDUMP,'(A)') 'end'
  WRITE(IDUMP,'(A)') '# ins,iew,iglg,nsta,nonl'
  DO JA = 1, N_REGIONS_NS
    DO JGL = D%NPTRFRSTLAT(JA), D%NPTRLSTLAT(JA)
      IGL = D%NFRSTLAT(JA) + (JGL - D%NPTRFRSTLAT(JA))
      DO JB = 1, N_REGIONS(JA)
        IF (D%NONL(JGL, JB) > 0) THEN
          WRITE(IDUMP,'(I0,A,I0,A,I0,A,I0,A,I0)') &
            JA, ',', JB, ',', IGL, ',', D%NSTA(JGL, JB), ',', D%NONL(JGL, JB)
        ENDIF
      ENDDO
    ENDDO
  ENDDO
  CLOSE(IDUMP)
  WRITE(0,'(A)') 'SUMP_TRANS: wrote grid_gp_decomp.csv'
ENDIF
