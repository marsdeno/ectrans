...
SUBROUTINE TGRL_COPY_PGLAT(PGLAT, YDBUFS, YLVARS, PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  !$OMP PARALLEL PRIVATE(JBLK,JFLD64,JK,IFLD,IFIRST,ILAST,IPOS,ZBUF)
  ALLOCATE(ZBUF(NPROMA, IFLDS))
#ifdef __NEC__
  ...
  DO JBLK=1,NGPBLKS
    ...
    IF(IFIRST > 0) THEN
      ILAST = KGPTRSEND(2,JBLK,MYSETW)
      IF (YDBUFS%LLTRGTOL) THEN
        ! TRGTOL: PGP(contiguous) -> ZBUF(L1) -> PGLAT(contiguous)
        IF(LLPGPONLY) THEN
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            ZBUF(1:(ILAST-IFIRST+1), JFLD64) = PGP(IFIRST:ILAST, IFLD, JBLK)
          ENDDO
        ELSE
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            IF(LLUV(IFLD)) THEN
              ZBUF(1:(ILAST-IFIRST+1), JFLD64) = PGPUV(IFIRST:ILAST, IUVLEVS(IFLD), IUVPARS(IFLD), JBLK)
            ELSEIF(LLGP2(IFLD)) THEN
              ZBUF(1:(ILAST-IFIRST+1), JFLD64) = PGP2(IFIRST:ILAST, IGP2PARS(IFLD), JBLK)
            ELSEIF(LLGP3A(IFLD)) THEN
              ZBUF(1:(ILAST-IFIRST+1), JFLD64) = PGP3A(IFIRST:ILAST, IGP3ALEVS(IFLD), IGP3APARS(IFLD), JBLK)
            ELSEIF(LLGP3B(IFLD)) THEN
              ZBUF(1:(ILAST-IFIRST+1), JFLD64) = PGP3B(IFIRST:ILAST, IGP3BLEVS(IFLD), IGP3BPARS(IFLD), JBLK)
            ELSE
        ...
        DO JK=IFIRST,ILAST
          ...
          DO JFLD64=1,IFLDS
            PGLAT(JFLD64,KINDEX(IPOS)) = ZBUF(JK-IFIRST+1,JFLD64)
          ENDDO
        ENDDO
      ELSE
        ! TRLTOG: PGLAT(contiguous) -> ZBUF(L1) -> PGP(contiguous)
        !DIR$ VECTOR ALWAYS
        DO JK=IFIRST,ILAST
          IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
          DO JFLD64=1,IFLDS
            ZBUF(JK-IFIRST+1,JFLD64) = PGLAT(JFLD64,KINDEX(IPOS))
          ENDDO
        ENDDO
        IF(LLPGPONLY) THEN
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            PGP(IFIRST:ILAST, IFLD, JBLK) = ZBUF(1:(ILAST-IFIRST+1), JFLD64)
          ENDDO
        ELSE
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            IF(LLUV(IFLD)) THEN
              PGPUV(IFIRST:ILAST, IUVLEVS(IFLD), IUVPARS(IFLD), JBLK) = ZBUF(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP2(IFLD)) THEN
              PGP2(IFIRST:ILAST, IGP2PARS(IFLD), JBLK) = ZBUF(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP3A(IFLD)) THEN
              PGP3A(IFIRST:ILAST, IGP3ALEVS(IFLD), IGP3APARS(IFLD), JBLK) = ZBUF(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP3B(IFLD)) THEN
              PGP3B(IFIRST:ILAST, IGP3BLEVS(IFLD), IGP3BPARS(IFLD), JBLK) = ZBUF(1:(ILAST-IFIRST+1), JFLD64)
            ELSE
