...
REAL(KIND=JPRB), ALLOCATABLE, SAVE :: ZBUF_PERSIST(:,:)
!$OMP THREADPRIVATE(ZBUF_PERSIST)
...
SUBROUTINE TGRL_COPY_ZCOMBUF(YDBUFS, YLVARS, INRS, ZCOMBUF, KPTRGP, PGP, PGPUV, PGP3A, PGP3B, PGP2, KF1, KF2)
  ...
  REAL(KIND=JPRB),OPTIONAL     :: PGP3B(:,:,:,:)
  REAL(KIND=JPRB),OPTIONAL     :: PGP2(:,:,:)
  INTEGER(KIND=JPIM) ,OPTIONAL, INTENT(IN) :: KF1, KF2
  ...
  LOGICAL            :: LL_TRGTOL
  INTEGER(KIND=JPIM) :: I_ISETW_INRS
  INTEGER(KIND=JPIM) :: I_JJ_OFF                ! (JJ-I_FLD_START)*IPOS
  INTEGER(KIND=JPIM) :: I_KPTRGP_IFLDT
  INTEGER(KIND=JPIM) :: I_LEV_UV, I_PAR_UV
  INTEGER(KIND=JPIM) :: I_PAR_GP2
  INTEGER(KIND=JPIM) :: I_LEV_GP3A, I_PAR_GP3A
  INTEGER(KIND=JPIM) :: I_LEV_GP3B, I_PAR_GP3B
  INTEGER(KIND=JPIM) :: JI_BASE                 ! JI = JI_BASE + JK
  ...
  IF (PRESENT(KF1)) I_FLD_START = KF1
  IF (PRESENT(KF2)) I_FLD_END   = KF2
  LL_TRGTOL   = YDBUFS%LLTRGTOL
  I_ISETW_INRS = ISETW(INRS)

  !$OMP PARALLEL DO SCHEDULE(STATIC) &
  !$OMP&  PRIVATE(IFLDT,IFIRST,ILAST,JK,JJ,JI,JBLK,I_JJ_OFF,JI_BASE, &
  !$OMP&          I_KPTRGP_IFLDT,I_LEV_UV,I_PAR_UV,I_PAR_GP2, &
  !$OMP&          I_LEV_GP3A,I_PAR_GP3A,I_LEV_GP3B,I_PAR_GP3B)
  DO JJ=I_FLD_START,I_FLD_END
    IFLDT    = IFLDA(JJ,INRS)
    I_JJ_OFF = (JJ-1)*IPOS
    ! Per-JJ hoists of the indirect field-index reads used inside DO JK.
    IF (LLINDER) THEN
      I_KPTRGP_IFLDT = KPTRGP(IFLDT)
    ELSEIF (.NOT. LLPGPONLY) THEN
      IF (LLUV(IFLDT)) THEN
        I_LEV_UV = IUVLEVS(IFLDT)
        I_PAR_UV = IUVPARS(IFLDT)
      ELSEIF (LLGP2(IFLDT)) THEN
        I_PAR_GP2 = IGP2PARS(IFLDT)
      ELSEIF (LLGP3A(IFLDT)) THEN
        I_LEV_GP3A = IGP3ALEVS(IFLDT)
        I_PAR_GP3A = IGP3APARS(IFLDT)
      ELSEIF (LLGP3B(IFLDT)) THEN
        I_LEV_GP3B = IGP3BLEVS(IFLDT)
        I_PAR_GP3B = IGP3BPARS(IFLDT)
      ENDIF
    ENDIF
    DO JBLK=1,NGPBLKS
      IFIRST = KGPTRSEND(1,JBLK,I_ISETW_INRS)
      IF(IFIRST > 0) THEN
        ILAST   = KGPTRSEND(2,JBLK,I_ISETW_INRS)
        JI_BASE = I_JJ_OFF + JPOS(JBLK,INRS) - IFIRST + 1
        IF(LLINDER) THEN
          IF (LL_TRGTOL) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              ZCOMBUF(JI,INRS) = PGP(JK,I_KPTRGP_IFLDT,JBLK)
            ENDDO
          ELSE
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PGP(JK,I_KPTRGP_IFLDT,JBLK) = ZCOMBUF(JI,INRS)
            ENDDO
          ENDIF
        ...
        ELSEIF(LLUV(IFLDT)) THEN
          IF (LL_TRGTOL) THEN
            ...
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              ZCOMBUF(JI,INRS) = PGPUV(JK,I_LEV_UV,I_PAR_UV,JBLK)
            ENDDO
          ELSE
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PGPUV(JK,I_LEV_UV,I_PAR_UV,JBLK) = ZCOMBUF(JI,INRS)
            ENDDO
          ENDIF
        ELSEIF(LLGP2(IFLDT)) THEN
          IF (LL_TRGTOL) THEN
            ...
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              ZCOMBUF(JI,INRS) = PGP2(JK,I_PAR_GP2,JBLK)
            ENDDO
          ELSE
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PGP2(JK,I_PAR_GP2,JBLK) = ZCOMBUF(JI,INRS)
            ENDDO
          ENDIF
        ELSEIF(LLGP3A(IFLDT)) THEN
          IF (LL_TRGTOL) THEN
            ...
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              ZCOMBUF(JI,INRS) = PGP3A(JK,I_LEV_GP3A,I_PAR_GP3A,JBLK)
            ENDDO
          ELSE
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PGP3A(JK,I_LEV_GP3A,I_PAR_GP3A,JBLK) = ZCOMBUF(JI,INRS)
            ENDDO
          ENDIF
        ELSEIF(LLGP3B(IFLDT)) THEN
          IF (LL_TRGTOL) THEN
            ...
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              ZCOMBUF(JI,INRS) = PGP3B(JK,I_LEV_GP3B,I_PAR_GP3B,JBLK)
            ENDDO
          ELSE
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              JI = JI_BASE + JK
              PGP3B(JK,I_LEV_GP3B,I_PAR_GP3B,JBLK) = ZCOMBUF(JI,INRS)
            ENDDO
          ENDIF
        ENDIF
      ENDIF
...
SUBROUTINE TGRL_COPY_PGLAT(PGLAT, YDBUFS, YLVARS, PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  ! Two-step tiled copy: gather into L1-resident buffer, then scatter
  ! Persistent per-thread ZBUF avoids OMP-private ALLOCATE/DEALLOCATE 
  !$OMP PARALLEL PRIVATE(JBLK,JFLD64,JK,IFLD,IFIRST,ILAST,IPOS)
  IF (.NOT. ALLOCATED(ZBUF_PERSIST) .OR. &
      SIZE(ZBUF_PERSIST,1) < NPROMA .OR. SIZE(ZBUF_PERSIST,2) < IFLDS) THEN
    IF (ALLOCATED(ZBUF_PERSIST)) DEALLOCATE(ZBUF_PERSIST)
    ALLOCATE(ZBUF_PERSIST(NPROMA, IFLDS))
  ENDIF
#ifdef __NEC__
  ...
  DO JBLK=1,NGPBLKS
    ...
    IF(IFIRST > 0) THEN
      ILAST = KGPTRSEND(2,JBLK,MYSETW)
      IF (YDBUFS%LLTRGTOL) THEN
        ! TRGTOL: PGP(contiguous) -> ZBUF_PERSIST(L1) -> PGLAT(contiguous)
        IF(LLPGPONLY) THEN
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64) = PGP(IFIRST:ILAST, IFLD, JBLK)
          ENDDO
        ELSE
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            IF(LLUV(IFLD)) THEN
              ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64) = PGPUV(IFIRST:ILAST, IUVLEVS(IFLD), IUVPARS(IFLD), JBLK)
            ELSEIF(LLGP2(IFLD)) THEN
              ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64) = PGP2(IFIRST:ILAST, IGP2PARS(IFLD), JBLK)
            ELSEIF(LLGP3A(IFLD)) THEN
              ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64) = PGP3A(IFIRST:ILAST, IGP3ALEVS(IFLD), IGP3APARS(IFLD), JBLK)
            ELSEIF(LLGP3B(IFLD)) THEN
              ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64) = PGP3B(IFIRST:ILAST, IGP3BLEVS(IFLD), IGP3BPARS(IFLD), JBLK)
            ELSE
        ...
        DO JK=IFIRST,ILAST
          ...
          DO JFLD64=1,IFLDS
            PGLAT(JFLD64,KINDEX(IPOS)) = ZBUF_PERSIST(JK-IFIRST+1,JFLD64)
          ENDDO
        ENDDO
      ELSE
        ! TRLTOG: PGLAT(contiguous) -> ZBUF_PERSIST(L1) -> PGP(contiguous)
        !DIR$ VECTOR ALWAYS
        DO JK=IFIRST,ILAST
          IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
          DO JFLD64=1,IFLDS
            ZBUF_PERSIST(JK-IFIRST+1,JFLD64) = PGLAT(JFLD64,KINDEX(IPOS))
          ENDDO
        ENDDO
        IF(LLPGPONLY) THEN
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            PGP(IFIRST:ILAST, IFLD, JBLK) = ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64)
          ENDDO
        ELSE
          DO JFLD64=1,IFLDS
            IFLD = IFLDOFF(JFLD64)
            IF(LLUV(IFLD)) THEN
              PGPUV(IFIRST:ILAST, IUVLEVS(IFLD), IUVPARS(IFLD), JBLK) = ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP2(IFLD)) THEN
              PGP2(IFIRST:ILAST, IGP2PARS(IFLD), JBLK) = ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP3A(IFLD)) THEN
              PGP3A(IFIRST:ILAST, IGP3ALEVS(IFLD), IGP3APARS(IFLD), JBLK) = ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64)
            ELSEIF(LLGP3B(IFLD)) THEN
              PGP3B(IFIRST:ILAST, IGP3BLEVS(IFLD), IGP3BPARS(IFLD), JBLK) = ZBUF_PERSIST(1:(ILAST-IFIRST+1), JFLD64)
            ELSE
