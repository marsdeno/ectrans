...
#ifdef HAVE_RAVE
CALL RAVE_BEGIN_REGION('INV_TRANS_CTL')
#endif
...
IF(NPROMATR > 0 .AND. IF_GPB > NPROMATR) THEN
 ...
 & KVSETUV,KVSETSC)
  ...
  DO JBLK=1,IBLKS
    ...
    #ifdef HAVE_RAVE
    CALL RAVE_BEGIN_REGION('LTINV_CTL')
    #endif
    ...
    #ifdef HAVE_RAVE
    CALL RAVE_END_REGION('LTINV_CTL')
    #endif

    IF(IF_UV_G > 0 .AND. IF_SCALARS_G > 0) THEN
      #ifdef HAVE_RAVE
      CALL RAVE_BEGIN_REGION('FTINV_CTL')
      #endif
      CALL FTINV_CTL(IF_UV_G,IF_SCALARS_G,&
       ...
       & PGP=PGP)
      #ifdef HAVE_RAVE
      CALL RAVE_END_REGION('FTINV_CTL')
      #endif
...
#ifdef HAVE_RAVE
CALL RAVE_END_REGION('INV_TRANS_CTL')
#endif
