...
USE TPM_ECTRANS_OPTS,ONLY : LUSE_OPT6, LREPORT_STAGE_TIME
USE TPM_DIM         ,ONLY : R
...
INTEGER(KIND=JPIM) :: IGLG,IRLEN,IPLAN_KLOT,IDUMP
...
REAL(KIND=JPRD) :: ZT_S158, ZT_S106, ZT_S1640, ZT_S_PLAN
REAL(KIND=JPRD), EXTERNAL :: TIMEF
REAL(KIND=JPRD), SAVE :: ZT_ACC_158 = 0.0_JPRD, ZT_ACC_106 = 0.0_JPRD
REAL(KIND=JPRD), SAVE :: ZT_ACC_1640 = 0.0_JPRD, ZT_ACC_PLAN = 0.0_JPRD
INTEGER(KIND=JPIM), SAVE :: ZT_NCALLS = 0
INTEGER(KIND=JPIM), SAVE :: I_NLOEN_SUM = -1
LOGICAL, SAVE :: LL_GRID_DUMPED = .FALSE.
...
IF (LREPORT_STAGE_TIME) ZT_S158 = TIMEF()
CALL GSTATS(158,0)
CALL TRGTOL(ZGTF,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
CALL GSTATS(158,1)
IF (LREPORT_STAGE_TIME) ZT_ACC_158 = ZT_ACC_158 + (TIMEF() - ZT_S158)

IF (LREPORT_STAGE_TIME) ZT_S106 = TIMEF()
CALL GSTATS(106,0)
...
IF (LREPORT_STAGE_TIME) THEN
  IF (I_NLOEN_SUM < 0 .AND. KF_FS > 0) THEN
    I_NLOEN_SUM = 0
    DO JGL = 1, D%NDGL_FS
      IGLG = D%NPTRLS(MYSETW) + JGL - 1
      I_NLOEN_SUM = I_NLOEN_SUM + G%NLOEN(IGLG)
    ENDDO
  ELSEIF (KF_FS == 0) THEN
    I_NLOEN_SUM = 0
  ENDIF

  ! One-time dump of grid decomposition (rank 1 only)
  IF (.NOT. LL_GRID_DUMPED .AND. MYPROC == 1) THEN
    LL_GRID_DUMPED = .TRUE.
    OPEN(NEWUNIT=IDUMP, FILE='grid_decomp.csv', STATUS='REPLACE', ACTION='WRITE')
    WRITE(IDUMP,'(A)') 'iglobal,nloen,nprocl_wave'
    DO JGL = 1, R%NDGL
      WRITE(IDUMP,'(I0,A,I0,A,I0)') JGL, ',', G%NLOEN(JGL), ',', D%NPROCL(JGL)
    ENDDO
    CLOSE(IDUMP)
    WRITE(0,'(A)') 'FTDIR: wrote grid_decomp.csv'
  ENDIF
...
IF (KF_FS > 0) THEN
  ...
  DO JGL = 1, D%NDGL_FS
    ...
    ENDIF
  ENDDO
  IF (LREPORT_STAGE_TIME) ZT_ACC_PLAN = ZT_ACC_PLAN + (TIMEF() - ZT_S_PLAN)

  IF (LREPORT_STAGE_TIME) ZT_S1640 = TIMEF()
  CALL GSTATS(1640, 0)
...
ENDIF
CALL GSTATS(1640, 1)
IF (LREPORT_STAGE_TIME) ZT_ACC_1640 = ZT_ACC_1640 + (TIMEF() - ZT_S1640)

CALL GSTATS(106,1)
IF (LREPORT_STAGE_TIME) THEN
  ZT_ACC_106 = ZT_ACC_106 + (TIMEF() - ZT_S106)
  ZT_NCALLS = ZT_NCALLS + 1
  IF (MYPROC == 1 .AND. ZT_NCALLS <= 10) THEN
    WRITE(0,'(A,F12.3,A,F12.3,A,F12.3,A,F12.3,A,I0,A,I0)') &
     & 'FTDIR wall: g158=', ZT_ACC_158/ZT_NCALLS, &
     & 'g106=', ZT_ACC_106/ZT_NCALLS, 'g1640=', ZT_ACC_1640/ZT_NCALLS, &
     & 'plan=', ZT_ACC_PLAN/ZT_NCALLS, &
     & ' ndgl=', D%NDGL_FS, ' nloen=', I_NLOEN_SUM
  ENDIF
ENDIF
