...
! Persistent per-thread tile buffer for the local PGLAT<->PGP copy.
! Avoids an OMP-private ALLOCATE/DEALLOCATE on every call.
...
! thread reads a scalar rather than re-derefing INDOFF per JL.
INTEGER(KIND=JPIM) :: I_KNDOFF_ISEND
INTEGER(KIND=JPIM) :: I_ISETW_INR
INTEGER(KIND=JPIM) :: I_JJ_OFF                ! (JJ-IRECV_FLD_START)*IPOS
...
!$OMP&          I_ISETW_INR,I_KPTRGP_IFLDT,I_LEV_UV,I_PAR_UV,I_PAR_GP2, &
!$OMP&          I_LEV_GP3A,I_PAR_GP3A,I_LEV_GP3B,I_PAR_GP3B)
  DO JJ=IRECV_FLD_START,IRECV_FLD_END
    IFLDT    = IFLDA(JJ,INR)
    I_JJ_OFF = (JJ-IRECV_FLD_START)*IPOS
    I_ISETW_INR = ISETW(INR)
    ...
    ENDIF
    DO JBLK=1,NGPBLKS
      IFIRST = KGPTRSEND(1,JBLK,I_ISETW_INR)
      IF(IFIRST > 0) THEN
        ILAST   = KGPTRSEND(2,JBLK,I_ISETW_INR)
        JI_BASE = I_JJ_OFF + JPOS(JBLK,INR) - IFIRST + 1
        IF(LLINDER) THEN
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PGP(JK,I_KPTRGP_IFLDT,JBLK) = PCOMBUFR(JI,INR)
          ENDDO
        ...
        ELSEIF(LLUV(IFLDT)) THEN
          ...
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PGPUV(JK,I_LEV_UV,I_PAR_UV,JBLK) = PCOMBUFR(JI,INR)
          ENDDO
        ELSEIF(LLGP2(IFLDT)) THEN
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PGP2(JK,I_PAR_GP2,JBLK) = PCOMBUFR(JI,INR)
          ENDDO
        ELSEIF(LLGP3A(IFLDT)) THEN
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PGP3A(JK,I_LEV_GP3A,I_PAR_GP3A,JBLK) = PCOMBUFR(JI,INR)
          ENDDO
        ELSEIF(LLGP3B(IFLDT)) THEN
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            JI = JI_BASE + JK
            PGP3B(JK,I_LEV_GP3B,I_PAR_GP3B,JBLK) = PCOMBUFR(JI,INR)
          ENDDO
        ENDIF
      ENDIF
    ENDDO
