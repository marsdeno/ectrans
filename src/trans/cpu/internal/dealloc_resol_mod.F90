...
USE TPM_FFTW        ,ONLY : DESTROY_PLANS_FFTW, EXPORT_FFTW_WISDOM
USE TPM_FLT         ,ONLY : S
USE TPM_CTL         ,ONLY : C
...
ELSE
  ...
  IF (NDEF_RESOL==0) THEN
    WRITE(NOUT,*) 'Exporting FFTW wisdom from DEALLOC_RESOL_MOD'
    CALL EXPORT_FFTW_WISDOM
  ELSE
    WRITE(NOUT,*) 'Not exporting FFTW wisdom yet, number of still active resols=',ndef_resol
  ENDIF
