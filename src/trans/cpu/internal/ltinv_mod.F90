...
USE TPM_ECTRANS_OPTS    ,ONLY : LUSE_OPT4
...
! Idea 9: separate handle for inner DR_HOOK regions (PRFI1B/VDTUV/SPNSDE/
...
IF (KF_UV > 0) THEN
  ...
  IVL   = 6*KF_UV+1
  IVU   = 8*KF_UV
  ! Idea 9: DR_HOOK around each PRFI1B/VDTUV/SPNSDE/LEINV/ASRE1B call
  ! to expose per-kernel cost in drhook.prof output.
  IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_VOR',0,ZHOOK_HANDLE_IN)
  CALL PRFI1B(KM,ZIA(:,IVORL:IVORU),PSPVOR,KF_UV,KFLDPTRUV)
  IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_VOR',1,ZHOOK_HANDLE_IN)
  IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_DIV',0,ZHOOK_HANDLE_IN)
  CALL PRFI1B(KM,ZIA(:,IDIVL:IDIVU),PSPDIV,KF_UV,KFLDPTRUV)
  IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_DIV',1,ZHOOK_HANDLE_IN)
  ILAST = ILAST+4*KF_UV

  IF (LHOOK) CALL DR_HOOK('LTINV_VDTUV',0,ZHOOK_HANDLE_IN)
  CALL VDTUV(KM,KF_UV,ZEPSNM,ZIA(:,IVORL:IVORU),ZIA(:,IDIVL:IDIVU),&
           & ZIA(:,IUL:IUU),ZIA(:,IVL:IVU))
  IF (LHOOK) CALL DR_HOOK('LTINV_VDTUV',1,ZHOOK_HANDLE_IN)
ENDIF
...
IF(KF_SCALARS > 0)THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SCA',0,ZHOOK_HANDLE_IN)
    CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSCALAR(:,:),KF_SCALARS,KFLDPTRSC)
    IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SCA',1,ZHOOK_HANDLE_IN)
  ELSE
    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IFIRST = ILAST+1
      ILAST  = IFIRST-1+2*NF_SC2
      IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC2',0,ZHOOK_HANDLE_IN)
      CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC2(:,:),NF_SC2)
      IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC2',1,ZHOOK_HANDLE_IN)
    ENDIF
    IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      ...
      DO J3=1,IDIM3
        ...
        IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC3A',0,ZHOOK_HANDLE_IN)
        CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC3A(:,:,J3),IDIM1)
        IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC3A',1,ZHOOK_HANDLE_IN)
      ENDDO
    ENDIF
    IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      ...
      DO J3=1,IDIM3
        IFIRST = ILAST+1
        ILAST  = IFIRST-1+2*IDIM1
        IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC3B',0,ZHOOK_HANDLE_IN)
        CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC3B(:,:,J3),IDIM1)
        IF (LHOOK) CALL DR_HOOK('LTINV_PRFI1B_SC3B',1,ZHOOK_HANDLE_IN)
      ENDDO
    ENDIF
  ENDIF
...
IF (KF_SCDERS > 0) THEN
  ...
  IF (LHOOK) CALL DR_HOOK('LTINV_SPNSDE',0,ZHOOK_HANDLE_IN)
  CALL SPNSDE(KM,KF_SCALARS,ZEPSNM,ZIA(:,ISL:ISU),ZIA(:,IDL:IDU))
  IF (LHOOK) CALL DR_HOOK('LTINV_SPNSDE',1,ZHOOK_HANDLE_IN)
ENDIF
...
IF( LATLON.AND.S%LDLL ) THEN
  ...
  IF( (S%LSHIFTLL .AND. KM < 2*IDGLU) .OR.&
   & (.NOT.S%LSHIFTLL .AND. KM < 2*(IDGLU-1)) ) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV_LDLL',0,ZHOOK_HANDLE_IN)
    CALL LEINV(KM,KMLOC,IFC,IIFC,KF_OUT_LT,ISL,IDGLU,ZIA(:,ISTA:ISTA+IFC-1),ZAOA1,ZSOA1)
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV_LDLL',1,ZHOOK_HANDLE_IN)
...
ELSE
  ...
  IF (LUSE_OPT4) THEN
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV_FUSED',0,ZHOOK_HANDLE_IN)
    CALL LEINV_ASRE_FUSED(KM,KMLOC,IFC,IIFC,KF_OUT_LT,ISL,IDGLU,&
     &                     ZIA(:,ISTA:ISTA+IFC-1))
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV_FUSED',1,ZHOOK_HANDLE_IN)
  ELSE
    ALLOCATE(ZAOA1(KDIM1,R%NLEI3))
    ALLOCATE(ZSOA1(KDIM1,R%NLEI3))
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV',0,ZHOOK_HANDLE_IN)
    CALL LEINV(KM,KMLOC,IFC,IIFC,KF_OUT_LT,ISL,IDGLU,ZIA(:,ISTA:ISTA+IFC-1),ZAOA1,ZSOA1)
    IF (LHOOK) CALL DR_HOOK('LTINV_LEINV',1,ZHOOK_HANDLE_IN)
...
!              --------------------------------------------
    ...
    IF (LHOOK) CALL DR_HOOK('LTINV_ASRE1B',0,ZHOOK_HANDLE_IN)
    CALL ASRE1B(KF_OUT_LT,KM,KMLOC,ZAOA1,ZSOA1)
    IF (LHOOK) CALL DR_HOOK('LTINV_ASRE1B',1,ZHOOK_HANDLE_IN)
    DEALLOCATE(ZAOA1)
    DEALLOCATE(ZSOA1)
  ENDIF
