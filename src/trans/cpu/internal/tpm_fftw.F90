...
!     WRITE(*,'("CREATE_PLAN_FFTW: END: DESTROYING A PLAN AT THE START OF THE LIST")')
    ENDIF
  ENDIF
  ! FFTW_MEASURE actually executes the plan during timing trials, so the dummy buffer
  ! must be sized for the real plan: ICDIST*KLOT complex = IRDIST*KLOT real (in-place).
  IF (JPRB == JPRD) THEN
  ...
  IF( KTYPE==1 )THEN
     ...
     IF (JPRB == JPRD) THEN
        CALL DFFTW_PLAN_MANY_DFT_C2R(IPLAN,IRANK,IN,KLOT,ZDUM,IEMBED,ISTRIDE,ICDIST,&
             & ZDUM,IEMBED,ISTRIDE,IRDIST,FFTW_MEASURE)
     ELSE
  ...
  ELSEIF( KTYPE==-1 )THEN
     ...
     IF (JPRB == JPRD) THEN
        CALL DFFTW_PLAN_MANY_DFT_R2C(IPLAN,IRANK,IN,KLOT,ZDUM,IEMBED,ISTRIDE,IRDIST,&
             & ZDUM,IEMBED,ISTRIDE,ICDIST,FFTW_MEASURE)
     ELSE
