...
SUBROUTINE SUMPLAT(KDGL,KPROC,KPROCA,KMYSETA,LDSPLIT,LDEQ_REGIONS,&
                   ...
                   &KPTRLS_FOURIER,KULTPP_FOURIER,LUSE_FOURIER_BOUNDARIES)
...
INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: KPTRLS_FOURIER(:)
INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: KULTPP_FOURIER(:)
LOGICAL, INTENT(IN), OPTIONAL :: LUSE_FOURIER_BOUNDARIES
...
IF (PRESENT(LUSE_FOURIER_BOUNDARIES) .AND. LUSE_FOURIER_BOUNDARIES) THEN
  IF (.NOT. PRESENT(KPTRLS_FOURIER) .OR. .NOT. PRESENT(KULTPP_FOURIER)) THEN
    CALL ABORT_TRANS ('SUMPLAT: LUSE_FOURIER_BOUNDARIES requires KPTRLS_FOURIER and KULTPP_FOURIER')
  ENDIF
  DO JA = 1, KPROCA
    IF (KULTPP_FOURIER(JA) > 0) THEN
      ILAST(JA) = KPTRLS_FOURIER(JA) + KULTPP_FOURIER(JA) - 1
    ELSE
      ILAST(JA) = KPTRLS_FOURIER(JA) - 1
    ENDIF
  ENDDO
  INDIC(:) = 0
  ITOT = INT(SUM(KLOEN(1:KDGL)), JPIB)
  KMEDIAP = INT(ITOT / INT(KPROCA, JPIB), JPIM)
  KRESTM = INT(ITOT - INT(KMEDIAP, JPIB) * INT(KPROCA, JPIB), JPIM)
  IF (KRESTM > 0) KMEDIAP = KMEDIAP + 1
  PMEDIAP = 0.0_JPRD
ELSEIF( LDEQ_REGIONS )THEN
...
IF (PRESENT(LUSE_FOURIER_BOUNDARIES) .AND. LUSE_FOURIER_BOUNDARIES) THEN
  DO JA = 1, KPROCA
    KPROCAGP(JA) = SUM(KLOEN(KFRSTLAT(JA):KLSTLAT(JA)))
  ENDDO
ENDIF
