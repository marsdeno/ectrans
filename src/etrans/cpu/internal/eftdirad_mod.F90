...
#ifdef WITH_FFT992
IF( TALD%LFFT992 .AND. T%LUSEFFT992(KGL) )THEN
...
ENDIF
#else
ELSEIF( TALD%LFFT992 )THEN
  IRLEN=G%NLOEN(IGLG)+R%NNOEXTZL
  ICLEN=(IRLEN/2+1)*2
  CALL BLUESTEIN_FFT(T%TB,IRLEN,ITYPE,KFIELDS,PREEL(1:KFIELDS,IOFF:IOFF+ICLEN-1))
ELSE
  CALL ABORT_TRANS('EFTDIRAD: no Fourier backend available in this build')
ENDIF
#endif
#else
  IRLEN=G%NLOEN(IGLG)+R%NNOEXTZL
